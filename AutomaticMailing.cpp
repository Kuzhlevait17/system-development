...
bool IsDayOff(int day, int month, int year)
{
    ...
    std::cout << "Ответ от API: " << response << std::endl;
...
bool IsFirstWorkingDayAfterBreak() 
{
    ...
    std::cout << "Today: " << today_d << "." << today_m << "." << today_y << " -> "
        << (todayIsDayOff ? "Day off" : "Working day") << std::endl;
    ...
    if (!todayIsDayOff) {
        ...
        std::cout << "Yesterday: " << yd << "." << ym << "." << yy << " -> "
            << (yesterdayIsDayOff ? "Day off" : "Working day") << std::endl;
